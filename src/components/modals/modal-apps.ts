...
import { delay, MOCK_APPS } from '../../utils/data'
...
export class ModalApps extends LitElement {
  ...
  renderContent() {
    ...
    return html` <div class="flex flex-col gap-3 max-h-[600px] overflow-auto">
      ${MOCK_APPS.map((app) => {
        return html` <div class="py-4 px-2 cursor-pointer flex items-center hover:bg-neutral-100">
          <div class="rounded-full bg-neutral-200 w-7 h-7 mr-2 flex items-center justify-center font-bold">
            <img src=${app.icon} alt=${app.name} class="rounded-full" />
          </div>
